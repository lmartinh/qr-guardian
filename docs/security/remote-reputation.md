...
## iOS Configuration
iOS reads the keys from `Info.plist`, which is populated from build settings in `iosApp/Configuration/Config.xcconfig`.

The tracked config file defines empty defaults and optionally includes a local override file:
...
For convenience, the repository includes `iosApp/Configuration/RemoteReputation.example.xcconfig` with empty placeholders only.
