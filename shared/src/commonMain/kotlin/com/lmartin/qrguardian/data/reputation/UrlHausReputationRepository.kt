...
class UrlHausReputationRepository(
    ...
    override suspend fun checkUrl(url: String): UrlReputationResult {
        ...
        return runCatching {
            val response = httpClient.post(URLHAUS_LOOKUP_URL) {
    ...
    private fun mapResponse(responseDto: UrlHausResponseDto): UrlReputationResult {
        return when (responseDto.queryStatus?.lowercase()) {
            "no_results" -> UrlReputationResult(
                status = UrlReputationStatus.Clean,
                provider = PROVIDER_NAME,
                categories = emptyList(),
                reasons = listOf("No threats were reported by URLhaus.")
            )
            "ok" -> when (responseDto.urlStatus?.lowercase()) {
                "online", "offline" -> UrlReputationResult(
            ...
            "invalid_url", "http_post_expected" -> unknownResult()
            else -> unknownResult()
        }
    }
    ...
    private fun unknownResult(): UrlReputationResult {
        return UrlReputationResult(
