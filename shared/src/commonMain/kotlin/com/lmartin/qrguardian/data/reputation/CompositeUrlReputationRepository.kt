...
class CompositeUrlReputationRepository(
    ...
    private fun merge(results: List<UrlReputationResult>): UrlReputationResult {
        ...
        val combinedProvider =
            when (distinctProviders.size) {
        ...
        val status =
            when {
                ...
                errors.isNotEmpty() && nonInformationalResults.isEmpty() &&
                    results.any {
                        it.status != UrlReputationStatus.NotConfigured
                    }
                -> UrlReputationStatus.Error

                results.all { it.status == UrlReputationStatus.NotConfigured } -> UrlReputationStatus.NotConfigured

                results.any { it.status == UrlReputationStatus.Error } &&
                    results.none {
                        it.status != UrlReputationStatus.Error && it.status != UrlReputationStatus.NotConfigured
                    } -> UrlReputationStatus.Error
